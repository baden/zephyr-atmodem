...
static const struct setup_cmd get_id_cmds[] = {
...
int query_ids(struct modem_data *mdata/*struct modem_context *mctx, struct k_sem *sem_response*/)
{
    /* Run query commands on the modem. */
	return modem_cmd_handler_setup_cmds(&mdata->mctx.iface, &mdata->mctx.cmd_handler,
					   get_id_cmds, ARRAY_SIZE(get_id_cmds),
...
char * modem_mdm_imei() {
    const struct device *dev = device_get_binding(DT_LABEL(DT_INST(0, simcom_sim7600g)));
