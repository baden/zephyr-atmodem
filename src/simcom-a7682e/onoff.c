 ...
 * return true if modem is already on after fw update
 */
static bool pin_init(const struct modem_config *config)
{
    bool status_state = false;

    // LOG_DBG("gsm_start: Setting up pins");
...
// Check if STATUS already high, then modem is already on after fw update

    if(gpio_pin_get_dt(&config->status)) {
        LOG_DBG("gsm_start: Modem already on. FW update?");
...
static const struct setup_cmd setup2_cmds[] = {
    ...
    SETUP_CMD_NOHANDLE("AT+CGEREP=1,0"),    // Network registration events
};
...
int simcom_setup(struct modem_data *mdata, const struct modem_config *config)
{
    ...
    init_stages.byte = 0;
	bool already_on = pin_init(config);
	...
	LOG_INF("Waiting for modem to respond (*ATREADY: 1 or any line?)");
	ret = k_sem_take(&mdata->sem_response, K_SECONDS(60) /*MDM_MAX_BOOT_TIME*/);
	if (ret < 0) {
		LOG_ERR("Timeout waiting for *ATREADY: 1");
        return ret;
	}
    ...
    ret = query_ids1(mdata);
    if (ret < 0) {
        k_sleep(K_SECONDS(1));
        ret = query_ids1(mdata); // try again
        if (ret < 0) {  // REALY?
            LOG_ERR("Error quering LTE ID's (%d)", ret);
            k_sleep(K_SECONDS(5));
            query_ids1(mdata); // try again
        }
		// goto error;
        return ret;
	}
    ...
    // // AT+CSSLCFG=?
    // modem_cmd_send(&mdata->mctx.iface, &mdata->mctx.cmd_handler, NULL, 0U, "AT+CSSLCFG=?", &mdata->sem_response, K_SECONDS(5));
    // k_sleep(K_SECONDS(1));

    // // AT+HTTPSSL=?
    // modem_cmd_send(&mdata->mctx.iface, &mdata->mctx.cmd_handler, NULL, 0U, "AT+HTTPSSL=?", &mdata->sem_response, K_SECONDS(5));
    // k_sleep(K_SECONDS(1));

    // // AT+HTTPPARA=?
    // modem_cmd_send(&mdata->mctx.iface, &mdata->mctx.cmd_handler, NULL, 0U, "AT+HTTPPARA=?", &mdata->sem_response, K_SECONDS(5));
    // k_sleep(K_SECONDS(1));

    #if 0
    ret = modem_http_init(mdata);
    if(ret < 0) {
        LOG_ERR("Error HTTP initialization.");
        return ret;
    }
    #endif
